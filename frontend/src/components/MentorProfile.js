...
import { Card, CardContent, CardMedia, Typography, Chip, Button, Box, Container, Grid } from "@mui/material";
...
const MentorProfile = () => {
  ...
  const mentor = {
    ...
    bio: "Passionate software engineer with 10+ years of experience in full-stack development. I love mentoring aspiring developers!",
    skills: ["React", "Node.js", "JavaScript", "Python", "Django"],
    availability: "Available for mentoring",
    image: "https://images.unsplash.com/photo-1526835746352-0b9da4054862?q=80&w=3087&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // Replace with actual image URL
    rating: 4.5, // Sample rating
  };

  const mentors = [
    { name: "Jane Smith", image: "https://randomuser.me/api/portraits/women/44.jpg", skills: ["Java", "Spring Boot"], rating: 4.7 },
    { name: "David Brown", image: "https://randomuser.me/api/portraits/men/46.jpg", skills: ["Machine Learning", "TensorFlow"], rating: 4.8 },
    { name: "Emily Clark", image: "https://randomuser.me/api/portraits/women/48.jpg", skills: ["UI/UX", "Figma"], rating: 4.6 },
  ];
...
/>
          ...
          <CardContent>
            ...
            {/* Skills Section */}
            <Box mt={2}>
              <Typography variant="subtitle1" fontWeight="bold" sx={{ "&:hover": { color: "blue" } }}>
                Skills
              </Typography>
        ...
        <Grid container spacing={3} justifyContent="center">
          {mentors.map((m, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}>
              <Card sx={{ textAlign: "center", padding: 2, borderRadius: 3, boxShadow: 2 }}>
                <CardMedia
                  component="img"
                  image={m.image}
                  alt={m.name}
                  sx={{ width: 100, height: 100, borderRadius: "50%", border: "3px solid white", display: "block", margin: "0 auto" }}
                />
                <CardContent>
                  <Typography variant="h6" fontWeight="bold">{m.name}</Typography>
                  <Box display="flex" justifyContent="center" gap={1} mt={1}>
                    {m.skills.map((skill, i) => (
                      <Chip key={i} label={skill} color="secondary" variant="outlined" sx={{
                      transition: "all 0.3s ease",
                      "&:hover": {
                        bgcolor: "#007BFF",
                        color: "#fff",
