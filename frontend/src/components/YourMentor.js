...
const MentorYour = () => {
  ...
  const mentorData = {
    ...
    bio: "Experienced full-stack developer with a passion for mentoring.",
      ...
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-lg p-6 h-[80vh] flex flex-col">
        ...
        <div className="border-b pb-4">
          <div className="flex flex-col items-center">
            ...
            <p className="text-sm text-gray-600 mt-1 text-center">{mentorData.bio}</p>
          </div>

          {/* Skills */}
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            {mentorData.skills.map((skill, index) => (
              <span key={index} className="bg-blue-100 text-blue-700 px-3 py-1 text-xs rounded">
                {skill}
              </span>
            ))}
          </div>
          ...
          <div className="mt-2 text-center">
            <p className="text-yellow-500 font-semibold">⭐ {mentorData.rating} / 5</p>
          </div>
        </div>

        {/* Scrollable Mentees Section */}
        <div className="flex-1 overflow-y-auto mt-4 px-2">
          <h3 className="text-lg font-semibold mb-3">Mentees Under {mentorData.name}</h3>
          {mentees.length === 0 ? (
            <p className="text-gray-500 text-center">No mentees assigned yet.</p>
          ) : (
            <div className="space-y-4">
              {mentees.map((mentee, index) => (
                <div key={index} className="flex items-center bg-gray-50 p-3 rounded-lg shadow-sm">
                  <img src={mentee.image} alt={mentee.name} className="w-12 h-12 rounded-full border mr-3" />
